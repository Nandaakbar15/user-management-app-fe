  ...
  const addUser = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://usermanagement-api.vercel.app/api/add-user",
        {
          nama: nama,
          email: email,
          nomorTelepon: noTelp,
            ...
            <Card>
              <CardContent>
                ...
                <form
                  onSubmit={addUser}
                  method="POST"
                  className="space-y-6 animate-slide-down"
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
                    <div>
                      <label
                        htmlFor="nama"
                        className="block text-sm font-medium font-sans text-gray-700 text-[16px]"
                      >
                        Nama
                      </label>
                      <input
                  ...
                  <div className="flex items-center gap-4 px-6 pb-6">
                    <button
                      type="submit"
                      className="bg-blue-600 text-white px-4 py-2 font-sans rounded-lg hover:bg-blue-700 transition"
                    >
                      Tambah
                    </button>
                  </div>
              ...
              <CardFooter>
                <Button variant={"secondary"}>
                  <Link to={"/user-list"}>Kembali</Link>
                </Button>
              </CardFooter>
            </Card>
          </div>
